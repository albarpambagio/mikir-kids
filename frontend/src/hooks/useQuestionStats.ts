...
async function fetchQuestionStats(
  ...
  if (shouldUseMockData()) {
    ...
    if (topicFilter && topicFilter !== "all") {
      filtered = filtered.filter(q => q.name === topicFilter)
    }

    // Map TopicStat to QuestionStats format
    return filtered.map(stat => ({
      topicId: stat.topicId,
      topic: stat.name,
      category: stat.category || "Unknown",
      grade: stat.grade || "Unknown",
      progress: stat.masteryLevel || 0,
      dueCount: stat.questionsDue || 0
    }))
  }
